...
- Claude background, tmux, agent-view management, cloud, remote-control, teleport, and built-in worktree modes pass directly to Claude and use Claude's own lifecycle; Myriad does not wrap or integrate them as managed tasks. `ultrareview` reserves and requires the exact current checkout instead of silently reviewing another snapshot. Outside Git, inside an already managed task, or with an explicit `--local`, the launcher runs Claude directly.
...
- `MYRIAD_WORKTREE` is the repository owned by the current managed task. Inspect, edit, validate, and commit that repository's intended result there. Stay on its assigned branch and leave its branch/worktree creation, integration, and cleanup to Myriad.
...
- Never erase unfinished work. Dirty or interrupted managed tasks are preserved, and a later ordinary `c` resumes preserved work when no Claude task is active. Ignored build artifacts from a successfully committed task are disposable and recorded before cleanup. Every ordinary managed task starts from the integration target, never from an arbitrary current feature branch. A task created with a manual-integration policy remains ready across reconciliation until explicitly integrated.
