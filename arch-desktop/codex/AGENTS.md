...
- Empty-branch fast-forward exception: before the first task-specific commit, a clean assigned branch may advance to the freshly fetched authoritative integration target with `git merge --ff-only FULL_TARGET_SHA` without another user confirmation. First verify that the worktree has no staged, unstaged, or untracked changes, no merge/rebase or Myriad reconciliation is in progress, the source ref is the established integration target, and `HEAD` is an ancestor of that exact commit with no task-unique commits. Run the command only in the assigned worktree, keep its branch name, and verify that the resulting `HEAD` matches the inspected target SHA. This permits no merge commit, rebase, rewritten task commit, manual change to the original checkout or integration target, or override of an actual Myriad refusal; integration and cleanup still use Myriad.
...
- In a managed task, Myriad assigns one branch and target for its repository. Stay on that branch for every commit there; never create, switch, merge, delete, or split branches in that assigned repository, except for the empty-branch fast-forward explicitly allowed in the managed-worktree policy above.
