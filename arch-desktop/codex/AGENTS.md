...
- In a Git repository, every ordinary `o` and `c` launch, including the first, uses the harness-managed task lifecycle rooted at the integration target. Claude and non-interactive tasks create their worktree immediately. Interactive Codex defers its branch and worktree only for inspection-only requests when the original checkout is clean and exactly at the recorded base; otherwise it provisions immediately. There is no repository mode setting and no contention-based native-checkout fallback. An ordinary launch resumes preserved work when no same-agent task is live; otherwise it creates a fresh managed task automatically. `o resume` explicitly resumes unfinished managed work or attaches a saved Codex chat to a managed task.
- Review commands reserve and require the exact current checkout instead of silently using another snapshot. Claude background, tmux, agent-view management, and built-in worktree modes pass directly to Claude and use Claude's own worktree lifecycle; the harness does not wrap or integrate them as `agent-task` tasks. Outside Git, inside an already managed task, or with an explicit `--local`, the launcher runs the agent directly.
...
- A fresh interactive managed Codex process starts before its Git worktree exists. The harness reserves the future path, and its trusted first-prompt hook selects a semantic slug plus read-only or mutating intent. A clean-base read-only task may inspect the original checkout without creating a branch. The first edit, unknown command, or potentially mutating shell command creates the branch and worktree synchronously, rejects that tool call, and must then be retried in `AI_TASK_WORKDIR`. Semantic branches use the slug directly and add a numeric suffix only on a real collision; attached repositories reuse that slug and resolve collisions independently. Never create, switch, or rename these branches yourself.
- A managed Codex process keeps the original checkout path as its logical cwd so saved-chat history remains repository-scoped. While the task is deferred, inspect there only and do not modify files or create artifacts. `AI_TASK_WORKTREE` and `AI_TASK_WORKDIR` identify the reserved isolated repository and original repository-relative starting directory before and after promotion. Once promoted, inspect, edit, validate, and commit the intended result there. Leave worktree integration and cleanup to the harness.
