...
- A managed Codex process keeps the original checkout path as its logical cwd so saved-chat history remains repository-scoped, but it must not inspect, modify, or create artifacts there. `MYRIAD_WORKTREE` and `MYRIAD_WORKDIR` identify the isolated repository and original repository-relative starting directory after first-prompt provisioning. Inspect, edit, validate, and commit the intended result there. Leave worktree integration and cleanup to Myriad.
...
- This remote synchronization exception permits no branch switching, destructive reset, manual rebase, force-push, edits to another checkout, or bypass of an actual Myriad refusal. Preserve the result and ask when upstream history was rewritten and merging would restore intentionally removed history, or when the intended target, ownership, or resolution remains materially unclear.
