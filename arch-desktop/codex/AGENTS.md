...
- A fresh interactive managed Codex process starts before its Git worktree exists. The harness reserves the future path, and its trusted first-prompt hook selects a semantic slug and creates the branch and worktree synchronously before the model turn starts. Semantic branches use the slug directly and add a numeric suffix only on a real collision; attached repositories reuse that slug and resolve collisions independently. Never create, switch, or rename these branches yourself.
- A managed Codex process keeps the original checkout path as its logical cwd so saved-chat history remains repository-scoped, but it must not inspect, modify, or create artifacts there. `AI_TASK_WORKTREE` and `AI_TASK_WORKDIR` identify the isolated repository and original repository-relative starting directory after first-prompt provisioning. Inspect, edit, validate, and commit the intended result there. Leave worktree integration and cleanup to the harness.
