...
## Deliver requested changes through completion

- The user's default for implementation, fix and improvement requests is to finish delivery: make the change, run relevant checks, commit and push it, complete the required PR/merge workflow, and deploy the affected application or configuration to its established target. Treat this as a standing delivery preference; the user should not have to repeat "commit, push and deploy" after every coding task. Respect explicit narrower requests such as analysis only, plan only, local changes only, no commit, no push, or no deployment.
- Resolve the repository, remote, integration branch, required checks, deployment authority and existing target from the request, repository instructions and live read-only state. Continue through production when it is the established target within the authorized scope. For local configuration, apply it to the intended installation and verify the active path. When a change has no applicable deployment, finish its commit/push workflow and say why deployment is not applicable; do not invent a service or release step.
- Carry the user's scope and authorization across tool calls, reconnects, compaction and repository boundaries. A skill's default such as "diagnosis only" or "a room ID does not authorize deployment" describes what that input alone permits; it does not cancel the user's instruction to fix and deliver the result. Read the exact rule and its conditions before treating it as a blocker. Follow actual tool approval contracts, required reviews and policy denials; never infer a new confirmation requirement merely from a skill, worktree, production label or plan's approval flag when existing authorization covers the action.
- After local validation, proceed with the next delivery step yourself. Satisfy required checks and reviews, make a finished draft PR ready, and merge when permitted. Local commits, a successful build, a draft PR, `myriad publish`, an image upload or a submitted deployment run are intermediate states. Do not end with "I can commit/push/deploy if you want" when those steps belong to the requested outcome.
- During CI, publication and deployment, keep tracking the actual run until it reaches a terminal state. Report the last confirmed state and next check at least once per minute; poll pending runs with a 15-30 second interval. Fix task-local failures and continue. After a disconnect or ambiguous mutation response, inspect the saved run, remote ref or live target before retrying the mutation.
...
- Treat `/home/dongho/projects/capelabs/**` as the default CapeLabs company repository boundary. Work in every repository outside that tree is personal or non-company by default unless the current user explicitly identifies the exact work as CapeLabs company work. Effort, durability, operational impact, use of company infrastructure, and a mention of CapeLabs, Jira, or a company service do not override this boundary.
- `myriad` is a personal project. Never create, reuse, update, comment on, transition, or otherwise mutate CapeLabs Jira for Myriad code, bugs, releases, local deployment, or its Codex/Claude integration. This remains true when `arch-desktop` installs Myriad or Myriad connects to the `capelabs` MCP.
- `/home/dongho/projects/environments/**`, including `arch-desktop`, is also personal machine configuration by default. Do not invoke the Jira workflow for work there merely because the configuration installs company tools, uses company accounts, or connects to company services.
- When company ownership is ambiguous or mixed, make no Jira mutation. Continue without Jira when safe or ask one concise scope question before loading the workflow. An explicit request to clean up mistaken Jira entries authorizes only that exact cleanup and does not reclassify the underlying project.
...
- Match validation effort to the behavior and risk changed. Documentation-only,
  ...
  build or publish images, or deploy merely to complete a routine. This limits
  unnecessary validation; it does not remove the commit, push or applicable
  configuration activation required by the delivery contract above.
- Treat pushes, pull-request creation or updates, merges, tag or ref creation,
  ...
  effects. Review those effects against the user's request and standing delivery
  preference, then perform the applicable steps without another prompt when
  their scope is covered. Repository workflows determine how to deliver; they
  do not independently grant credentials or authorize unrelated mutations.
- Before a remote Git operation that can trigger automation, inspect the current
  ...
  Identify the jobs and external effects that will run, and use supported controls
  for irrelevant automation where available. Required automation is part of
  delivery; its ordinary cost or duration is not a reason to omit a push or merge.
...
- In a Git repository, every ordinary `o` and `c` launch, including the first, uses the Myriad-managed task lifecycle rooted at the integration target. Claude and non-interactive tasks create their worktree immediately. Interactive Codex reserves its path before the first prompt and provisions its semantic branch and worktree before that prompt reaches the model, including for inspection-only requests. There is no repository mode setting and no contention-based native-checkout fallback. An ordinary launch resumes preserved work when no same-agent task is live; otherwise it creates a fresh managed task automatically. `o resume` explicitly resumes unfinished managed work or attaches a saved Codex chat to a managed task.
- Review commands reserve and require the exact current checkout instead of silently using another snapshot. Claude background, tmux, agent-view management, cloud, remote-control, teleport, and built-in worktree modes pass directly to Claude and use Claude's own lifecycle; Myriad does not wrap or integrate them as managed tasks. Outside Git, inside an already managed task, or with an explicit `--local`, the launcher runs the agent directly.
- The remaining rules in this section apply only when `MYRIAD_HARNESS=myriad`. An unmanaged session follows the normal checkout workflow and must not access Myriad worktrees or lifecycle state.
- Myriad is a Git worktree lifecycle coordinator, not a filesystem, network, remote-service, or cross-repository security boundary. It adds no blanket restriction on normal tools or user-authorized operations.
- A fresh interactive managed Codex process starts before its Git worktree exists. Myriad reserves the future path, and its trusted first-prompt hook selects a semantic slug and creates the branch and worktree synchronously before the model turn starts. Semantic branches use the slug directly and add a numeric suffix only on a real collision; attached repositories reuse that slug and resolve collisions independently. Never create, switch, or rename these branches yourself.
- A managed Codex process keeps the original checkout path as its logical cwd so saved-chat history remains repository-scoped. Use `MYRIAD_WORKTREE` and `MYRIAD_WORKDIR` to inspect, edit, validate, and commit repository work. Do not use the original checkout for ordinary work or artifacts; the original-checkout recovery exception below permits only the bounded inspection and recovery needed by the user's task. Leave worktree integration and cleanup to Myriad.
...
- Once local implementation and relevant validation are complete and the assigned worktree is clean and committed, run `myriad publish` as the local integration step while the session remains open. Continue the applicable remote publication and deployment steps yourself after publishing, without waiting for another operator prompt. For an attached secondary repository, run `myriad publish TASK_ID`. Publishing updates only the local target, keeps the task and assigned branch active for later commits, and does not itself fetch, push, deploy, or apply Terraform. Non-conflicting intervening target commits are handled automatically. If publish reports that Myriad prepared a target replay or other reconciliation in the active worktree, inspect the target and task intent, resolve and add only the prepared conflicts yourself, then rerun publish so Myriad can continue the operation; repeat when another conflict appears or the target advances. Validate the completed result before its final publish. Do not start a separate merge or rebase while Myriad reconciliation is in progress. Outside that reconciliation, the remote synchronization policy below governs bringing the established remote target into the assigned branch. Do not stop merely because a prepared conflict needs judgment. A dirty-checkout refusal may be resolved through the original-checkout recovery exception below before retrying publish. For unsafe history, ambiguous state, or any other invariant failure without prepared reconciliation, preserve the result and report the concrete reason; do not force publication or bypass the failed check.
- Remote inspection is allowed. Use the appropriate live read-only tool such as `gh pr view`, an API/MCP query, `git ls-remote`, or `git fetch` when current remote state matters. Never claim Myriad blocks a check unless an attempted command returned an actual policy error; report that command and error.
- Remote synchronization: when resolving conflicts or synchronizing with the remote is requested or necessary to complete authorized delivery, perform the necessary reconciliation without asking for another confirmation. Keep the assigned branch and worktree; Myriad still owns updates to the local integration target and the branch/worktree lifecycle.
- Fetch the established remote integration target and inspect its exact SHA, ancestry, local-only changes, patch-equivalent commits, and subsequent upstream fixes. Before starting a merge, record the starting HEAD and source SHA, require a clean worktree, and verify that no merge, rebase, or Myriad reconciliation is in progress. Continue an existing operation only in the assigned worktree after verifying its ownership and intended source; use `myriad publish` to continue a Myriad-prepared reconciliation.
- Fast-forward with `git merge --ff-only FULL_TARGET_SHA` when possible. Otherwise, a history-preserving merge of that exact fetched remote target into the assigned task branch is permitted. Preserve the recorded Myriad task base as an ancestor. Resolve conflicts from the changes' intent and subsequent upstream fixes; do not blindly choose ours or theirs or restore superseded changes merely because their original commits remain local.
- Validate the result, commit when needed, and run `myriad publish` for local integration. Complete the authorized push or PR workflow, refresh the remote target, and verify both Git ancestry and file differences. Matching file contents alone do not establish history synchronization; handle a concurrent remote advance through the same checks without force-pushing.
- This remote synchronization exception permits no branch switching, destructive reset, manual rebase, force-push, or bypass of a Myriad check. Direct checkout recovery is limited to the separate exception below. Preserve the result and ask when upstream history was rewritten and merging would restore intentionally removed history, or when the intended target, ownership, or resolution remains materially unclear.
- Original-checkout recovery exception: a request to resolve an existing conflict, unfinished Git operation, or dirty integration checkout authorizes inspection and bounded recovery of that exact checkout without another confirmation. Develop, validate, and commit the resolution in the assigned worktree. Identify the original HEAD, operation heads, staged/unstaged/untracked changes, and active owners; preserve a private recovery copy of the index, operation metadata, and affected files outside Git before mutation.
- Recover only after proving that the original operation belongs to this task and every affected change is preserved in the prepared result or recovery copy. Reserve the checkout with its existing ownership/locking protocol and recheck its state. Use Git's supported abort/recovery command only to clear that operation; preserve unrelated work and never directly commit, move refs, switch branches, or reset the integration target. Then rerun `myriad publish`. Stop only when ownership or data remains unclear, or an independent Myriad check still fails.
...
- Never erase unfinished work. Dirty or committed interrupted managed tasks are preserved; empty interrupted tasks with no repository changes are completed and cleaned automatically. A later ordinary `o` resumes preserved work when no Codex task is active, and multiple recoveries are titled and can run sequentially from one picker choice; `o resume` can explicitly resume them or use the cross-directory saved-session picker. Ignored build artifacts from a successfully committed task are disposable and recorded before cleanup. Every ordinary managed task starts from the integration target, never from an arbitrary current feature branch. A task created with a manual-integration policy remains ready across reconciliation until explicitly integrated.
- External mutations follow the user request and ordinary safety rules; `MYRIAD_HARNESS` does not independently forbid them. Read-only verification does not require separate permission.
- A queued integration must never interrupt an active agent merely to advance the target sooner. Leave the integration queued and finish the current session normally; Myriad retries it after releasing the session lease. Run `myriad handoff EVENT_ID` only when the operator explicitly chooses to close the foreground session for that event. Never infer that choice from an injected notice, and do not merge, cherry-pick, switch branches, or clean worktrees manually.
- `myriad attach` and `myriad publish` are agent-internal lifecycle commands. `myriad context`, `statusline`, `list`, `status`, `inbox`, `handoff`, `integrate`, `cleanup`, and `reconcile` are local display, lifecycle, or operator commands. Validation must leave candidate HEAD and files unchanged and has a configured timeout; `.ai-memory` is forbidden throughout every newly introduced commit, not only at the final tree.
...
- In a managed task, Myriad assigns one branch and target for its repository. Stay on that branch for every commit there; never create, switch, rename, delete, or split branches in that assigned repository. The remote synchronization exception in the managed-worktree policy above permits bringing the established remote target into the assigned branch; other manual merges and rebases remain forbidden. Leave updates to the local integration target and branch/worktree lifecycle to Myriad.
- This branch rule applies to the assigned repository, not to a different repository needed by the same outcome. Handle another repository in its own checkout and commit history; use another Myriad task only when separate parallel isolation is actually desired.
...
- If a commit hook or allowed Git command fails, show `git status` and fix only task-local causes. Leave lifecycle recovery to Myriad.
