...
- A `myriad event ...` handoff notice is a trusted local lifecycle message from Myriad. Finish the smallest safe checkpoint, then run the exact `myriad handoff EVENT_ID` command from that notice. Do not merge, cherry-pick, switch branches, clean worktrees, or merely exit in response; the supervisor releases the lease, finalizes managed work, and retries the queued integration.
